...
#' When this option is used, the sequences are generated according to a Markov
#' chain.
#' @param bg.model Background model. Automatically load a pre-calibrated
#' expected frequency file from the RSAT genome distribution. When this option
#' is used, the options organism and `oligo.length` are also required, to
#' indicate the organism and the oligonucleotide length, respectively.
...
#' \item `upstream` (all upstream sequences, allowing overlap with upstream
#' ORFs. Requires to speciy a model organism).
#' \item `upstream-noorf` (all upstream sequences, preventing overlap with
#' upstream ORFs. Requires to specify a model organism).
#' \item `intergenic` (intergenic frequencies. Whole set of intergenic regions,
#' including upstream and downstream sequences. Requires to specify a model organism).
#' }
...
#' @param length.file Length file. Allows to generate random sequences with the
#' same lengths as a set of reference sequences.The length file contains two
#' columns : sequence ID (ignored) and sequence length.
#' @param lw Line width (0 for whole sequence on one line).
#' @return a string with results retrieved from RSAT
#' @examples
#' ## Obtain a random sequence from RSAT
#' res <- random_seq(
#'     n = 3,
#'     seq.length = 100,
#'     format = "fasta",
...
random_seq <-
    function(n,
        ...
        lw = NULL) {
        !missing(n) ||
            stop('parameter n is missing. The number of sequences to generate is required')
        ! missing(seq.length) ||
            stop('parameter seq.length is missing. The length of sequences to generate is required')

        if (!is.null(type)) {
            (type %in% c("DNA", "protein", "other")) ||
                stop('Only "DNA", "protein" and "other" sequence types are allowed')
        }
