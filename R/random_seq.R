...
#' @examples
#' res <- random_seq(n = 3, seq.length = 100, format = "fasta",
#'                 type = "DNA", seed = 12,
...
random_seq <- function(n, seq.length, format = "fasta", type = NULL, seed = NULL,
                      alphabet = NULL, expfreq = NULL, bg.model = NULL,
                      organism = NULL, oligo.length = NULL,
                      length.file = NULL, lw = NULL){

  !missing(n) || stop('parameter n is missing. The number of sequences to generate is required')
  !missing(seq.length) || stop('parameter seq.length is missing. The length of sequences to generate is required')
