...
#' @return A dataframe with the following columns corresponding to the TFBSs
...
get_binding_sites <- function(regulondb, TF, seq.format = "table") {
    ...
    if (is.null(tfbs.raw)) {
        ...
        ## convert raw info into a table with 1 row per TFBS and 1 col
        ## per attribute
        tfbs.table <-
            as.data.frame(strsplit(x = tfbs.raw$tfbs_unique, split = " "),
                col.names = "V1")
        tfbs.table <-
            t(as.data.frame(strsplit(
                x = as.character(tfbs.table$V1), split =  "\t"
            )))
        ...
        if (seq.format == "fasta") {
            ...
            stop("seq.format must be 'table' or 'fasta' ", call. = FALSE)
        }
