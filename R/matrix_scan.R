...
#' @param matrix Matrix/ces to scan with. The matrix format is specified with the option `matrix.format` (see below) Default format: `tab`.
...
#' For `background.input` and `background_window`,  the pseudo frequency is automatically calculated with the length (L) of the sequence following this formula:
#' square-root of L divided by L+squareroot of L.
#' For `background.model`,  default value is 0.01.
#' If the training sequence length (L) is known,  the value can be set by `background.pseudo` option to square-root of L divided by L+squareroot of L.
...
#' \item `occ_sig_rank`
#' }
#' @param uth Upper threshold on some parameter. Format=list of 'param value'. Supported parameters: same as `lth`.
...
matrix_scan <- function(sequence, matrix, sequence.format = "fasta", matrix.format = "tab",
