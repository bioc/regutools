...
xx <- get_dataset( regulondb=ecoli_regulondb, 
            dataset="GENE",
            attributes=c("posleft", "posright","name"), 
            list("name"=c("araC","crp","lacI")) )

xx <- get_dataset( regulondb=ecoli_regulondb, 
            dataset="GENE", 
            attributes=c("name", "dna_sequence", "product_sequence"),
...
xx <- xx[!is.na(xx$dna_sequence),]

pr <- DNAStringSet( xx$dna_sequence )

mcols( pr ) <- xx[,c("name"), drop=FALSE]
