...
test_that( "Functions to retrieve gene regulation work as expected", {
  ...
  rs2 <- get_gene_regulators( regdb, c("araC","fis","crp"), format="onerow" )
  expect_s4_class( rs2, "regulondb_result" )
  expect_true( metadata( rs2 )$format == "onerow" )
  rs3 <- get_gene_regulators( regdb, c("araC","fis","crp"), format="table" )
  ...
  expect_s4_class( get_regulatory_summary( regdb, gene_regulators = c("araC", "modB") ), "regulondb_result")
} )
