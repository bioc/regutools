...
test_that("Test that the logical conditions its made as expected", {
  ...
  condi_interval <- build_condition(regdb, dataset = "GENE",
  ...
  condi_2_filters_as_interval <- build_condition(regdb, dataset = "GENE",
                                    filters = list(posright=c("2000","40000")
...
expect_match(condi_2_filters_as_interval,"<=")
  ...
  expect_warning(build_condition(regdb, dataset = "GENE",
                                    filters = list(posright=c("2000","40000","50000"),name=c("ara")),
                                    operator= NULL,
                                    interval= "posright",
                                    partialmatch = NULL), "Only the first two values of interval will be considered.")

  #Having only one value
  expect_error(build_condition(regdb, dataset = "GENE",
                                 filters = list(posright=c("2000")),
                                 operator= NULL,
                                 interval= "posright",
                                 partialmatch = NULL), "Two values in the interval filter are required. ")

#Diferent operators
  condi_operar_and <- build_condition(regdb, dataset = "GENE",
...
condi_no_interval <- build_condition(regdb, dataset = "GENE",
                                    filters = list(posright=c("2000","40000")),
                                    operator= NULL,
                                    interval= "posright",
                                    partialmatch = NULL)

  condi_operar_or <- build_condition(regdb, dataset = "GENE",
                                      filters = list(name=c("ara"),
                                                     strand=c("forward")),
                                      operator= "OR",
                                      interval= NULL,
  ...
  expect_error(build_condition(regdb, dataset = "GENE",
                               ...
                               operator= NULL,
                               interval= "posrigh",
                               partialmatch = NULL),"The argument filters is not a list")
